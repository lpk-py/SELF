   ...
   ! Set the initial guess and the source terms
     ...
     do iP = 0,pDeg
        do iS = 0,pDeg
           ...
           thisx = thisCGSM % geometry % x(iS,iP)
           thisy = thisCGSM % geometry % y(iS,iP)

           thisCGSM % source(iS,iP) = -8.0_prec*pi*pi*cos(2.0_prec*pi*thisX)*sin(2.0_prec*pi*thisY)

           diags(iS,iP,1) = cos(2.0_prec*pi*thisX)*sin(2.0_prec*pi*thisY)
     ...
     CALL thisCGSM % CALC_GRADIENT( diags(:,:,1), diags(:,:,2), diags(:,:,3), pDeg, pDeg )

     CALL thisCGSM % MAPPED_LAPLACIAN( diags(:,:,1), diags(:,:,3) )
