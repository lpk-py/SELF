MODULE CGSEM_1D_FEM_PRECONDITIONER
!  CGSEM_1D_FEM_PRECONDITIONER.f90
!
!  This module defines a data-type for a preconditioner to the matrix system
!  arrived at in CGSEM_1D_CLASS.f90. The preconditioner can be used to accelerate
...
!  using the global quadrature nodes as the grid. To build the preconditioner, only the mesh 
!  is needed. For each equation, we only need to know the grid spacing on either side of a node.
!  However, for a small cost in memory the grid spacing in a single array along with the coefficients
!  of the FEM preconditioner.
!  
!
!
! Module History 
...
!
!
END MODULE CGSEM_1D_FEM_PRECONDITIONER
