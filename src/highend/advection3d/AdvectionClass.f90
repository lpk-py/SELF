...
!
 ...
 ! DECLARATIONS
   ...
   REAL(prec), INTENT(out)      :: rFac(1:myDGSEM % nEq)
      ...
      CALL myDGSEM % relax(iEl) % GetSolutionAtNode( i, j, k, Fac )
...
!$OMP DO
      ...
      CALL myDGSEM % clock % StartThisTimer( tID )
...
! 
 ...
 ! DECLARATIONS
   ...
   ! Local
   INTEGER :: k, nEq
   INTEGER :: e1, s1, e2, s2, iError, nS, nP
      ...
      DO iQ = 0, nQ
         DO iP = 0,nP ! Loop over the y-points
            DO iS = 0,nS ! Loop over the x-points
               ...
               sContFlux(iS,:) = (dydp*dzdq - dzdp*dydq)*xF + &
            ...
            DO iEq = 1, myDGSEM % nEq
               sContFluxDer(0:nS,iEq) = DGSystemDerivative( nS, dMatX, qWeightX, fL(iEq), fR(iEq), &
                                                            sContFlux(0:nS,iEq), lwest, least  )
            ENDDO
     
            DO iS = 0, nS ! Loop over the x-points
               DO iEq = 1, myDGSEM % nEq  ! Loop over the number of equations
                  tend(iS, iP, iQ, iEq) = -sContFluxDer(iS,iEq)
         ...
         DO iS = 0,nS ! Loop over the x-points
            DO iP = 0,nP ! Loop over the y-points
               ...
               !And now the contravariant flux
               pContFlux(iP,:) = (dydq*dzds - dzdq*dyds)*xF + &
            ...
            DO iEq = 1, myDGSEM % nEq
               pContFluxDer(0:nP,iEq) = DGSystemDerivative(  nP, dMatY, qWeightY, fL(iEq), fR(iEq), &
                                                             pContFlux(0:nP,iEq), lsouth, lnorth  )
            ENDDO

         
            DO iP = 0, nP 
               DO iEq = 1, myDGSEM % nEq  
                  tend(iS, iP, iQ, iEq) = tend(iS, iP, iQ iEq)- pContFluxDer(iP,iEq)
               ENDDO 
            ENDDO 
      ...
      DO iP = 0 ,nP
         DO iS = 0, nS
            DO iQ = 0, nQ
               ...
               !And now the contravariant flux
               qContFlux(iQ,:) = (dyds*dzdp - dzds*dydp)*xF + &
            ...
            ! At this x-level, calculate the y-DG-advective derivative
            DO iEq = 1, myDGSEM % nEq
               qContFluxDer(0:nQ,iEq) = DGSystemDerivative(  nQ, dMatZ, qWeightY, fL(iEq), fR(iEq), &
                                                             qContFlux(0:nP,iEq), lbottom, ltop  )
            ENDDO

         
            DO iP = 0, nP 
               DO iEq = 1, myDGSEM % nEq  
                  tend(iS, iP, iQ, iEq) = tend(iS, iP, iQ iEq)- qContFluxDer(iP,iEq)
               ENDDO 
            ENDDO 
